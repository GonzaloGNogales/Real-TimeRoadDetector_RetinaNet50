...
def plot_metrics_legend(history, case, v_loss, v_acc):
    if history is not None:
        ...
        plt.legend()
        plt.savefig('./metrics/' + case + '/' + 'Train and Validation Accuracy MultiClassClassifier.png')
        ...
        ax_loss.text(0.95, 0.5, 'val_loss = {:.6f}'.format(v_loss), transform=ax_loss.transAxes, fontsize=12,
                     ...
                     verticalalignment='center', bbox=props)
        plt.legend()
        plt.savefig('./metrics/' + case + '/' + 'Train and Validation Loss MultiClassClassifier.png')
